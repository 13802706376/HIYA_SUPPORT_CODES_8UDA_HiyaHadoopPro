...
public class HiyaDeptEarliestEmpMapper extends Mapper<LongWritable, Text, Text, Text>
{
	// 用于缓存 dept文件中的数据
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
	{
		...
		if (deptCache.containsKey(kv[7]))
		{
			...
			{
				context.write(new Text(deptCache.get(kv[7].trim())), new Text(kv[1].trim() + "," + kv[4].trim()));
			}
		}
