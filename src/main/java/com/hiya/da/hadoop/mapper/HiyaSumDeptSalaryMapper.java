...
public class HiyaSumDeptSalaryMapper extends Mapper<LongWritable, Text, Text, Text>
{
	// 用于缓存 dept文件中的数据
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
	{
		...
		if (deptCache.containsKey(kv[7]))
		{
			...
			{
				String deptName = deptCache.get(kv[7].trim());
				String salVal = kv[5].trim();
				hiyaLog.info("HiyaSumDeptSalaryMapper>map>deptName="+deptName);
				hiyaLog.info("HiyaSumDeptSalaryMapper>map>salVal="+salVal);
				context.write(new Text(deptName), new Text(salVal));
			}
		}
	}
}
